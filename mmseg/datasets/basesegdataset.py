...
class BaseSegDataset(BaseDataset):
    ...
    Args:
        ...
        max_images (int, optional): Maximum number of images to load. Defaults to None.
        subset_file (str, optional): File containing a list of allowed filenames. Defaults to None.
    """
    ...
    def __init__(
        ...
        img_suffix=".jpg",
        seg_map_suffix=".png",
        tube_suffix="_combined.pt",
        metainfo: Optional[dict] = None,
        data_root: Optional[str] = None,
        data_prefix: dict = dict(img_path="", seg_map_path="", tube_path=""),
        ...
        self.tube_suffix = tube_suffix
        self.ignore_index = ignore_index
        self.reduce_zero_label = reduce_zero_label
        self.backend_args = backend_args.copy() if backend_args else None
        self.max_images = max_images
        self.subset_file = subset_file
        self.subset_basenames = None
        if self.subset_file is not None:
            # Load list of allowed filenames from subset file
            self.subset_basenames = set()
            with open(self.subset_file, "r") as f:
                for line in f:
                    basename = line.strip()
                    # Remove extension if present
                    basename = osp.splitext(basename)[0]
                    self.subset_basenames.add(basename)
    ...
    def load_data_list(self) -> List[dict]:
        ...
        tube_dir = self.data_prefix.get("tube_path", None)
        if not osp.isdir(self.ann_file) and self.ann_file:
            ...
            for line in lines:
                if self.max_images is not None and len(data_list) >= self.max_images:
                    break
                img_name = line.strip()
                ...
                # Skip if not in subset_basenames (if subset filtering is enabled)
                if (
                    self.subset_basenames is not None
                    and basename not in self.subset_basenames
                ):
                    continue
                ...
                if tube_dir is not None:
                    tube = img_name + self.tube_suffix
                    data_info["tube_path"] = osp.join(tube_dir, tube)
            ...
            for img in fileio.list_dir_or_file(
                ...
                # Get basename without extension
                basename = osp.splitext(osp.basename(img))[0]
                ...
                if ann_dir is not None:
                    ...
                    data_info["seg_map_path"] = osp.join(ann_dir, seg_map)
                if tube_dir is not None:
                    tube = img[:-_suffix_len] + self.tube_suffix
                    data_info["tube_path"] = osp.join(tube_dir, tube)
                data_info["label_map"] = self.label_map
                data_info["reduce_zero_label"] = self.reduce_zero_label
                data_info["seg_fields"] = []
