...
class LoadAnnotations(MMCV_LoadAnnotations):
    ...
    def _load_seg_map(self, results: dict) -> None:
        ...
        # Verify if tube_path exists and load the tube tensor
        if "tube_path" in results:
            tube_path = results["tube_path"]
            if tube_path and os.path.exists(tube_path):
                tube_tensor = np.load(tube_path)
                results["tube_tensor"] = tube_tensor["data"].transpose(1, 2, 0)
            else:
                print(f"Warning: Tube tensor file not found at {tube_path}")
            results["seg_fields"].append("tube_tensor")
        return results
