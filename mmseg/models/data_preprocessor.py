...
class SegDataPreProcessor(BaseDataPreprocessor):
    ...
    def forward(self, data: dict, training: bool = False) -> Dict[str, Any]:
        ...
        inputs_zoom_out = [
            data.get("data_samples", None)[i].get("img_zoom_out", None)
            for i in range(len(data_samples))
        ]
        # TODO: whether normalize should be after stack_batch
        if self.channel_conversion and inputs[0].size(0) == 3:
            inputs = [_input[[2, 1, 0], ...] for _input in inputs]
            if inputs_zoom_out[0] is not None:
                inputs_zoom_out = [
                    _input_zoom_out[[2, 1, 0], ...]
                    for _input_zoom_out in inputs_zoom_out
                ]

        inputs = [_input.float() for _input in inputs]
        if inputs_zoom_out[0] is not None:
            inputs_zoom_out = [
                _input_zoom_out.float() for _input_zoom_out in inputs_zoom_out
            ]
        ...
        if self._enable_normalize:
            ...
            if inputs_zoom_out[0] is not None:
                inputs_zoom_out = [
                    (_input_zoom_out - self.mean) / self.std
                    for _input_zoom_out in inputs_zoom_out
                ]
        ...
        if training:
            if inputs_zoom_out[0] is not None:
                for i in range(len(data_samples)):
                    data_samples[i].img_zoom_out = inputs_zoom_out[i]
